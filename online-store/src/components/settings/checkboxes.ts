...
class Checkbox implements ICheckboxes {
    ...
    window.addEventListener("beforeunload", () => {
      if (Reset.isSaveAllowed) {
        localStorage.setItem(
          "savedCheckboxes",
          JSON.stringify(Checkbox.currentCheckboxes)
        );
      }
    });
  }
  ...
  getCheckbox(): void {
    if (localStorage.getItem("savedCheckboxes") !== null) {
      Checkbox.currentCheckboxes = JSON.parse(
        localStorage.getItem("savedCheckboxes") as string
      );
    }
  }
  ...
  createCheckbox(
    ...
    checkbox.addEventListener("click", () => {
      if (checkbox.checked) {
        if (
          TemplateKeeper.currentTemplate[prop].length ===
          TemplateKeeper.defaultTemplate[prop].length
        ) {
        ...
        if (TemplateKeeper.currentTemplate[prop].length !== 1) {
          TemplateKeeper.currentTemplate[prop] = TemplateKeeper.currentTemplate[
            prop
          ].filter((item) => item != text);
        } else {
          TemplateKeeper.currentTemplate[prop] =
            TemplateKeeper.defaultTemplate[prop];
        }
        Checkbox.currentCheckboxes = Checkbox.currentCheckboxes.filter(
          (item) => item != name
        );
      }
